    ...
    public function sendOtpEmail(string $email, string $otp): bool
    {
        try {
            ...
            $mailMessage = (new MailtrapEmail())
                ->from(new Address('hello@demomailtrap.co', 'EduKids Support'))
                ->to(new Address($email))
                ->subject('Your Password Reset Code - EduKids')
    ...
    public function sendPasswordResetConfirmation(string $email, string $userName = ''): bool
    {
        try {
            ...
            $mailMessage = (new MailtrapEmail())
                ->from(new Address('hello@demomailtrap.co', 'EduKids Support'))
                ->to(new Address($email))
                ->subject('Password Changed Successfully - EduKids')
...
<body>
    <div class="email-container">
        ...
        <div class="email-body">
            ...
            <p>We received a request to reset your password for your EduKids account. Below is your verification code:</p>
            ...
            <ol>
                <li>Visit the password reset page on EduKids</li>
            ...
            <p>For security reasons, never share this code with anyone, including EduKids staff.</p>

            <p>Best regards,<br>
            <strong>The EduKids Team</strong></p>
        </div>
...
<body>
    <div class="email-container">
        ...
        <div class="email-body">
            ...
            <p>You can now log in to EduKids using your new password.</p>
...
HTML;
    ...
    /**
     * Send email verification link to user via Mailtrap
     */
    public function sendVerificationEmail(string $email, string $verificationUrl, string $userName = ''): bool
    {
        try {
            $this->logger->info('📧 [MAILTRAP] Building verification email', [
                'recipient_email' => $email,
                'timestamp' => date('Y-m-d H:i:s'),
            ]);

            $mailMessage = (new MailtrapEmail())
                ->from(new Address('hello@demomailtrap.co', 'EduKids'))
                ->to(new Address($email))
                ->subject('Verify Your Email - EduKids')
                ->html($this->getVerificationEmailTemplate($email, $verificationUrl, $userName))
            ...
            if ($isSuccessful) {
                $this->logger->notice('🎉 [MAILTRAP] Verification email sent!', ['email' => $email]);
            }

            return $isSuccessful;
        } catch (\Throwable $e) {
            $this->logger->error('❌ [MAILTRAP] Failed to send verification email', [
                'exception_message' => $e->getMessage(),
                'email' => $email,
            ]);
     ...
     */
    private function getVerificationEmailTemplate(string $email, string $verificationUrl, string $userName): string
    {
...
<head>
    ...
    <style>
        ...
        .email-header { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: #ffffff; text-align: center; padding: 30px; }
        .email-header h1 { margin: 0; font-size: 24px; }
        .email-body { padding: 30px; color: #333; line-height: 1.6; }
        .verify-btn { display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: #ffffff; text-decoration: none; padding: 14px 40px; border-radius: 5px; font-size: 16px; font-weight: bold; margin: 20px 0; }
        .email-footer { background: #f8f9fa; text-align: center; padding: 20px; font-size: 12px; color: #666; }
    </style>
</head>
<body>
    <div class="email-container">
        ...
        <div class="email-body">
            ...
            <p>Thank you for registering with EduKids! Please verify your email address by clicking the button below:</p>
            ...
            <p>Best regards,<br><strong>The EduKids Team</strong></p>
        </div>
        <div class="email-footer">
            <p>&copy; 2026 EduKids. All rights reserved.</p>
            <p>Account Email: {$email}</p>
        </div>
    </div>
</body>
